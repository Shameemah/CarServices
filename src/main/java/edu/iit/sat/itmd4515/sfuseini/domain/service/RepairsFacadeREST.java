...
@Stateless
@Path("/repairs")
public class RepairsFacadeREST extends AbstractFacade<Repairs> {

    @PersistenceContext(unitName = "itmd4515PU")
    private EntityManager em;

    public RepairsFacadeREST() {
        super(Repairs.class);
    }
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Repairs entity) {
        super.edit(entity);
    }
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Repairs> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
